...
# Logistic
#
p1 <- plot_transition(final_logistic, "name_country", areas = "Rwanda") +
  guides(fill = "none") +
  ylim(c(0, 0.25)) +
...
p2 <- plot_smoother(final_logistic, areas = "Rwanda") +
  guides(fill = "none") +
  ylim(c(-0.3, 0.35)) +
...
p3 <- plot_indicator(final_logistic, areas = "Rwanda", color_sources = TRUE) +
...
# Spline
#
p4 <- plot_transition(final_spline, "name_country", areas = "Rwanda") +
  guides(fill = "none") +
  ylim(c(0, 0.25)) +
  labs(x = expression(eta[ct] / P^u[c]), y = expression(f[b])) +
  my_theme +
...
p5 <- plot_smoother(final_spline, areas = "Rwanda") +
  guides(fill = "none") +
  ylim(c(-0.3, 0.35)) +
  labs(x = "Year", y = expression(epsilon[ct])) +
  my_theme +
...
p6 <- plot_indicator(final_spline, areas = "Rwanda", color_sources = TRUE) +
  scale_shape_discrete(name = "Data Source", drop = FALSE) +
  guides(fill = "none") +
  labs(x = "Year", y = expression(eta[ct])) +
  scale_x_continuous(breaks = seq(1970, 2030, 10)) +
...
p6$layers[[6]]$mapping <- aes(y = contraceptive_use_modern, shape = source)
